...
// Dark mode: same structure, inks lifted for a near-black ground; the 3D stock keeps its Aspire blue.
const INK_DARK={ ink:'#dbe4ee', sel:'#ff8a3d', annot:'#5ad19a',
  node:'#f0b04a', nodeSmooth:'#4ad98a', handle:'#6fb3ff',
  snap:'#ffd27a', draft:'#ffd27a', marquee:'rgba(255,154,60,0.85)',
  measure:'#7fd0ff', measureBg:'rgba(20,26,34,0.92)', measureInk:'#dbe4ee',
  drill:'#5ad19a', cut:'#5ad19a', rapid:'rgba(170,170,170,0.5)',
  legendBd:'rgba(255,255,255,0.25)', legendInk:'#cdd6e2', legendSub:'#8fa2ba',
...
const THEMES_DARK={
  ...
  preview: Object.assign({}, INK_DARK, { bg:'#2a2c5a', gradTop:'#22245a', gradBot:'#3a3d80',
    stockTop:[56,67,245], stockDeep:[14,18,104],
    grid:'rgba(255,255,255,0.05)', axis:'rgba(140,160,255,0.3)',
    jobFace:'rgba(56,67,245,0.92)', jobShadow:'rgba(0,0,20,0.5)',
    jobEdge:'#5a64e8', jobKey:'rgba(200,205,255,0.35)', jobCorner:'#a8aeff',
    labelBg:'rgba(30,32,70,0.96)', labelBd:'#6b74c8', labelInk:'#e0e4ff' })
};
...
// Layer colours were picked for a white ground; on the dark ground lift any colour too dark to read (keeps the hue).
function layerInk(c){ if(!darkMode) return c; const m=/^#([0-9a-f]{6})$/i.exec(c||''); if(!m) return c;
  const v=parseInt(m[1],16); let r=(v>>16)&255, g=(v>>8)&255, b=v&255; const lum=(0.299*r+0.587*g+0.114*b)/255;
  if(lum>=0.45) return c; const t=0.62; r=Math.round(r+(255-r)*t); g=Math.round(g+(255-g)*t); b=Math.round(b+(255-b)*t);
  return '#'+((1<<24)|(r<<16)|(g<<8)|b).toString(16).slice(1); }
// ---- rulers (X along the top, Y down the left; a marker follows the cursor) ----
const RULER_W=20;
let rulers=(function(){ try{ return localStorage.getItem('aqcam.rulers')!=='0'; }catch(e){ return true; } })();
function setRulers(on){ rulers=!!on; try{ localStorage.setItem('aqcam.rulers', rulers?'1':'0'); }catch(e){} document.body.classList.toggle('rulers', rulers); render(); drawRulerMarks(lastScr); }
function rulerStep(){ const want=64/view.ppi;   // inches per major label so labels sit >= 64px apart
  const steps=[0.0625,0.125,0.25,0.5,1,2,4,5,10,20,50,100]; for(const st of steps){ if(st>=want) return st; } return 100; }
function drawRulers(){ const th=TH(); const W=cv.width, H=cv.height, R=RULER_W;
  const major=rulerStep(), minor=major/4;
  ...
  for(let x=Math.floor(w0.x/minor)*minor; x<=w1.x; x+=minor){ const sx=Math.round(W2S({x,y:0}).x)+0.5; if(sx<R)continue;
    const isMajor=Math.abs(x/major-Math.round(x/major))<1e-6; const len=isMajor?R:(Math.abs(x/(major/2)-Math.round(x/(major/2)))<1e-6?R*0.55:R*0.3);
    ctx.moveTo(sx,R-len); ctx.lineTo(sx,R); if(isMajor) ctx.fillText(fmt(x), sx+2, 1); }
  for(let y=Math.floor(w0.y/minor)*minor; y<=w1.y; y+=minor){ const sy=Math.round(W2S({x:0,y}).y)+0.5; if(sy<R)continue;
    const isMajor=Math.abs(y/major-Math.round(y/major))<1e-6; const len=isMajor?R:(Math.abs(y/(major/2)-Math.round(y/(major/2)))<1e-6?R*0.55:R*0.3);
    ctx.moveTo(R-len,sy); ctx.lineTo(R,sy);
    if(isMajor){ ctx.save(); ctx.translate(1,sy-2); ctx.rotate(-Math.PI/2); ctx.textAlign='left'; ctx.textBaseline='top'; ctx.fillText(fmt(y),0,0); ctx.restore(); } }
  ctx.stroke();
  ctx.fillStyle=th.rulerBg; ctx.fillRect(0,0,R,R); ctx.fillStyle=th.rulerInk; ctx.font='8px monospace'; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText('in', R/2, R/2);
  ctx.restore(); }
// cursor marker on the overlay canvas (cheap: no scene re-render on every mouse move)
let lastScr=null; const hctx=overlay.getContext('2d');
function drawRulerMarks(scr){ if(overlay.width!==cv.width||overlay.height!==cv.height){ overlay.width=cv.width; overlay.height=cv.height; }
  hctx.clearRect(0,0,overlay.width,overlay.height); if(!rulers||viewMode==='preview'||!scr) return;
  const th=TH(); hctx.strokeStyle=th.rulerMark; hctx.lineWidth=1.5; hctx.beginPath();
  if(scr.x>RULER_W){ hctx.moveTo(scr.x,0); hctx.lineTo(scr.x,RULER_W); } if(scr.y>RULER_W){ hctx.moveTo(0,scr.y); hctx.lineTo(RULER_W,scr.y); } hctx.stroke(); }
...
function drawShape(s, selected, dimmed){
  ...
  const th=TH();
  const col = selected ? th.sel : (isDim ? th.annot : layerInk(doc.layers.get(s.layer)?.color || th.ink));
  ctx.save();
...
const MENU_ACTIONS={
  ...
  'rulers':()=>{ setRulers(!rulers); syncViewMenu(); }, 'dark':()=>{ setTheme(!darkMode); syncViewMenu(); },
  'fit':()=>fitAll(), 'fitjob':()=>fitJob(),
  'keys':()=>{document.getElementById('keysModal').style.display='block';},
  'selftest':()=>selfTest()
};
function syncViewMenu(){ const set=(act,on)=>{ const b=document.querySelector('.mdrop button[data-act="'+act+'"] .chk'); if(b) b.textContent=on?'✓':''; }; set('rulers',rulers); set('dark',darkMode); }
...
function setView(mode){
  ...
  drawRulerMarks(viewMode==='preview'?null:lastScr);
}
...
function wire(){
  ...
  initHelpSearch();
  on('btnTheme',()=>{ setTheme(!darkMode); syncViewMenu(); });
  setTheme(darkMode); document.body.classList.toggle('rulers', rulers); syncViewMenu();
...
window.AQ_STUDIO = { doc, get sel(){return sel;}, get view(){return viewMode;}, CADCORE, CAM, importText, importPDF, openProject, saveProject, saveProjectAs, projectJSON, setView, camBuild, setTool, addShapes, render,
  ...
  get gl3d(){return gl3d;}, setTheme, setRulers, get darkMode(){return darkMode;}, get rulers(){return rulers;}, gl3dPreset, get simField(){return simField;}, openHelpSearch, helpSearchRun, showForm, runSim, recalcAll, get opsQueue(){return opsQueue;} };
