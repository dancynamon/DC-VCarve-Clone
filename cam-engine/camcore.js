...
function profileOp(contours, opts){
  ...
  const seq = o.order==='optimize' ? orderContours(contours, o.orderStart) : contours;
  let prevEntry = o.orderStart || {x:0,y:0}, prevDir = null;   // entry:'nearest' chains each entry to the last one
  for(const c of seq){
    let loops;
    if(!c.closed){
      ...
      let src = c.pts;
      if(o.entry==='serpentine' && prevDir){
        // Successive parallel cuts alternate direction, so each starts where the last
        // ended. Strict nearest-end agrees almost always but flips on a near-tie (89.53"
        // vs 89.71" on lgc-50-board-1's last cut); alternation is the actual strategy and
        // matches 8/8 across boards 1 and 5.
        const a=src[0], b=src[src.length-1];
        const d={x:b.x-a.x, y:b.y-a.y};
        if(d.x*prevDir.x + d.y*prevDir.y > 0) src=reversed(src);
      } else if(o.entry==='nearest' || o.entry==='serpentine'){
        const a=src[0], b=src[src.length-1];
