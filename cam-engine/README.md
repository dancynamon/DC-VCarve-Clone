...
- **Dimension annotations (B3):** the **Dim** tool (D) annotates a measured **linear** (aligned / horizontal / vertical — click 2 points then the offset), **radial** (radius / diameter — click a circle or arc), or **angular** (click vertex then two rays) dimension. Each dimension carries extension lines, a dimension line, filled arrowheads, and an auto-updating label (value re-derived live as endpoints move with the Node tool; `°`/`⌀`/precision/unit aware). Dimensions are annotations — never emitted to CAM (excluded from `shapesToContoursInput`), round-trip in the `.aqcam` project, and export as geometry. Pure core: `cadcore.mkDim` / `dimMeasure` / `dimLabel` / `dimGeometry`.
...
- **CAM built in (Op selector):**
  ...
  - **Inlay (C5)** — a paired **female cavity + male plug** from the same contours. The female is a pocket cleared to the nominal outline; the male is the same shape **undersized by a Gap"** (fit/glue clearance, cut as an outside profile) so the plug drops into the cavity with an even gap all round, with an optional **Mirror** (flip the plug board to seat it). Part selector = Female / Male / Both. Pure core: `camcore.inlayOp` (reuses `pocketOp` + `offsetLoop` + `profileOp`).
