...
- **Anchor-based position (VCarve-style 9-box):** every shape dialog (create or edit) has an anchor picker — lower left / lower middle / lower right / center left / center / center right / upper left / upper middle / upper right — and X/Y are the world position of that point on the shape's bounding box. Switching the anchor re-expresses X/Y without moving the shape; editing size or rotation keeps the anchor pinned. The last anchor is remembered (`aqcam.anchor`) and is also the point that snaps when you drag-move a selection. Core: `ANCHORS` / `anchorPoint` / `bboxAnchor` / `moveAnchorTo` / `moveGroupAnchorTo`.
- **Create by numbers:** with a shape tool active (rect / round / circle / ellipse / polygon / star / text) press **Enter** or click "Create by numbers…" — the dialog opens on a default shape parked at the job's matching anchor point, live-previews, and **Create** adds it (TTF outline text is traced and anchored as a group).
